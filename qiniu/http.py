...
def _post(url, data, files, auth):
    if _session is None:
        _init()
    try:
        tm = config.get_default('connection_timeout')
        r = _session.post(url, data=data, files=files, auth=auth, headers=_headers, timeout=tm)
...
class _TokenAuth(AuthBase):
    ...
    def __call__(self, r):
        r.headers['Authorization'] = 'QBox {0}'.format(self.token)
